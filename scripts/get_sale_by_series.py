...
def get_db_connection():
    ...
    Reuses standard database configuration patterns across deterministic scripts.
    """
...
def get_sale_by_series(series_code: str) -> dict:
    """
    Deterministic query function to retrieve Sales (VSALE) transactions by Voucher Series code.

    Validated dimension relationship:
        public.transection.series -> public.mst_series.ser_code
    ...
    series_code_clean = str(series_code).strip()

    conn = get_db_connection()
    try:
        conn.set_session(readonly=True)
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # STEP 1 — VALIDATE SERIES
            cur.execute("""
                SELECT 
                    TRIM(CAST(ser_code AS TEXT)) AS ser_code,
                    TRIM(CAST("NAME" AS TEXT)) AS series_name
            ...
            series_name = s_row["series_name"]

            # STEP 2 — FIND VSALE INVOICES
            cur.execute("""
            ...
            # STEP 3 — ACCOUNTING SUMMARY (Calculated directly from transection to avoid JOIN multiplication)
            cur.execute("""
                SELECT 
            ...
            total_series_amount = float(ac_summary["total_amount"]) if ac_summary else 0.0
            ...
            cur.execute("""
                ...
                ORDER BY inv_no;
            """, (series_code_clean, inv_nos))
            inv_summary_rows = {r["inv_no"]: r for r in cur.fetchall()}

            # STEP 5 & STEP 6 — OPERATIONAL SALES CONTEXT (Preserve complete invoice context)
            cur.execute("""
                SELECT DISTINCT
            ...
            # Group vehicle context by invoice
            inv_vehicles_map = {}
            ...
            return {
                ...
                "total_amount": total_series_amount,
                "invoices": invoices
            }
    finally:
        conn.close()
...
if __name__ == "__main__":
    # STEP 10 — TEST BLOCK
    result = get_sale_by_series("0000001")
    print(f"Series: {result.get('series_code')}")
    print(f"Series Name: {result.get('series_name')}")
