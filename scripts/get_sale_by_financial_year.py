...
def get_sale_by_financial_year(co_year: str) -> dict:
    """
    Deterministic query function to retrieve Sales (VSALE) transactions by Financial Year.

    Authoritative relationship:
        public.transection.co_year -> public.mst_year.co_year
    ...
    co_year_clean = str(co_year).strip()

    conn = get_db_connection()
    try:
        conn.set_session(readonly=True)
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # STEP 1 — VALIDATE CO_YEAR AGAINST mst_year
            cur.execute("""
                SELECT 
                    TRIM(CAST(co_year AS TEXT)) AS co_year
                FROM public.mst_year
                WHERE co_year IS NOT NULL AND TRIM(CAST(co_year AS TEXT)) = TRIM(CAST(%s AS TEXT));
            """, (co_year_clean,))
            y_row = cur.fetchone()

            if not y_row:
                return {
                    "co_year": co_year_clean,
                    "exists": False,
                    "number_of_invoices": 0,
                    "number_of_accounting_rows": 0,
                    "total_amount": 0,
            ...
            # STEP 2 — FIND VSALE INVOICES FOR REQUESTED FINANCIAL YEAR
            cur.execute("""
            ...
            total_year_amount = float(ac_summary["total_amount"]) if ac_summary else 0.0
            ...
            return {
                ...
                "total_amount": total_year_amount,
...
if __name__ == "__main__":
    result = get_sale_by_financial_year("2025-26")
    print(f"Financial Year: {result.get('co_year')}")
